...
轻量、自托管的私网服务导航面板。首页只展示服务入口，设备和服务维护集中放在设置菜单中，适合部署在 NAS、软路由或家庭服务器内网。

![Dashboard 首页预览](docs/assets/dashboard-preview.png)

## 功能范围

- 设备新增、编辑、删除，维护设备地址、类型、图标和说明。
- 服务新增、编辑、删除、启用、停用、手动检测和点击卡片新标签打开。
...
- 服务卡片支持自定义图标标识、图片 URL，以及常见自托管服务 Logo 关键字。
- GitHub Actions 自动运行后端测试、前端构建，并在推送 `main` 或版本标签时构建多架构 Docker 镜像发布到 GHCR。
